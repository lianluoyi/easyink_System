...
@NoArgsConstructor
@AllArgsConstructor
public class CustomerIdFilterModel {
    /**
     * 是否过滤判断
     */
     ...
     * 客户id列表
     */
    private Set<String> filterIdList = new HashSet<>();

    /**
     * 数据添加, 取交集
     * @param dataIdList
     */
    public void addAndConditionFilter(Set<String> dataIdList) {
        ...
        if (CollectionUtils.isEmpty(getFilterIdList())) {
            ...
            Set<String> origins = getFilterIdList();
            Set<String> temp = new HashSet<>();
            // 取交集
            for (String current : dataIdList) {
     ...
     * @param dataIdList
     */
    public void addOrConditionFilter(Set<String> dataIdList) {
        this.filterFlag = true;
        if (CollectionUtils.isEmpty(dataIdList)) {
            return;
        }
