...
public class WeChatContactMapping extends RootEntity implements Comparable<WeChatContactMapping> {
    ...
    public int compareTo(WeChatContactMapping weChatContactMapping) {
        if (ObjectUtils.isEmpty(this.finalChatContext) || ObjectUtils.isEmpty(weChatContactMapping.getFinalChatContext())) {
            return -1;
        } else if (!this.finalChatContext.containsKey(WeConstans.MSG_TIME) || ObjectUtils.isEmpty(this.finalChatContext.getLong(WeConstans.MSG_TIME))
                || !weChatContactMapping.getFinalChatContext().containsKey(WeConstans.MSG_TIME) || ObjectUtils.isEmpty(weChatContactMapping.getFinalChatContext().getLong(WeConstans.MSG_TIME))) {
            return -1;
        } else if (this.finalChatContext.getLong(WeConstans.MSG_TIME) > (weChatContactMapping.getFinalChatContext().getLong(WeConstans.MSG_TIME))) {
            return -1;
        } else if (this.finalChatContext.getLong(WeConstans.MSG_TIME).equals(weChatContactMapping.getFinalChatContext().getLong(WeConstans.MSG_TIME))) {
            return 0;
        }
        return 1;
    }
