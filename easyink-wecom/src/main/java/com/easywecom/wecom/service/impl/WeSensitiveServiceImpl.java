...
import com.easywecom.common.core.domain.elastic.ElasticSearchEntity;
...
import com.easywecom.common.core.elasticsearch.ElasticSearch;
...
import com.easywecom.wecom.domain.query.WeSensitiveHitQuery;
import com.easywecom.wecom.domain.vo.WeUserBriefInfoVO;
import com.easywecom.wecom.domain.vo.WeUserVO;
import com.easywecom.wecom.login.util.LoginTokenService;
import com.easywecom.wecom.mapper.WeSensitiveMapper;
...
public class WeSensitiveServiceImpl implements WeSensitiveService {
    ...
    @Override
    public void hitSensitive(String corpId, List<JSONObject> entityList) {
        ...
        for (JSONObject obj : entityList) {
            if ("send".equals(obj.getString(WeConstans.ACTION)) && "text".equals(obj.getString(WeConstans.MSG_TYPE))) {
                textList.add(obj);
            }
        }
        ...
        for (JSONObject jsonObject : entityList) {
            if (minTimeStamp == 0L) {
                minTimeStamp = jsonObject.getLong(WeConstans.MSG_TIME);
                continue;
            }
            if (jsonObject.getLong(WeConstans.MSG_TIME) < minTimeStamp) {
                minTimeStamp = jsonObject.getLong(WeConstans.MSG_TIME);
            }
        }
                ...
                if (org.apache.commons.lang3.StringUtils.isNotBlank(j.getString("roomid"))) {
                    ...
                    if (weGroup != null) {
                        ...
                        String roomAvatar = weGroupMemberDtos.stream()
                                ...
                                .filter(com.easywecom.common.utils.StringUtils::isNotEmpty)
                                .limit(9)
    ...
    private void sendMessage(List<ElasticSearchEntity> list, WeSensitive weSensitive) {
        if (WeConstans.SENSITIVE_NOTICE.equals(weSensitive.getAlertFlag()) && CollectionUtils.isNotEmpty(list)) {
            ...
            for (ElasticSearchEntity entity : list) {
                ...
                String content = "员工%s发送的消息包含敏感词“%s”，请进入easyWeCom【会话存档】的消息审计查看完整消息";
